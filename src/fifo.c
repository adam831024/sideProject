...
uint16_t FifoCount(Fifo_t *fifo)
{
	...
	//return ((fifo->End - fifo->Begin) + NPI_TL_BUF_SIZE) % NPI_TL_BUF_SIZE;
}

uint8_t FifoPrePop( Fifo_t *fifo , uint16_t offset)
{
	uint16_t i = fifo->Begin + offset;
	return  fifo->Data[FifoNext( fifo, i )];
}
